* Omnifun
  ...
  This is a fork from `omniconf` which makes all the applicable functions take an
  component as the first argument so you are able to have multiple configurations
  at the same time. README was updated with the new usages.
...
(cfg/define config-comp
  {:hostname {:description "where service is deployed"
   ...
   2. Populate the configuration from available sources:
      ...
      #+BEGIN_SRC clojure
      (cfg/populate-from-cmd config-comp args) ;; args is a command-line arguments list
      (when-let [conf (cfg/get config-comp :conf)]
        (cfg/populate-from-file config-comp conf))
   ...
   4. Use =get= to extract arbitrary value from the configuration.
      ...
      #+BEGIN_SRC clojure
      (cfg/get config-comp :database :ip)
      (cfg/get config-comp [:database :ip])
      #+END_SRC
      ...
      #+BEGIN_SRC clojure
      (cfg/set config-comp :database :port 3306)
      (cfg/set config-comp [:database :port] 3306)
      #+END_SRC
...
** Configuration scheme syntax
   ...
   - =:type= --- currently the following types are supported: =:string=,
     ...
     : (cfg/define config-comp {:foo {:type :boolean}, :bar {:type :boolean}})
     : ...
   ...
   - =:default= --- the option will be initialized with this value. The default
     ...
     the user. Note that you must invoke =(cfg/enable-functions-as-defaults config-comp)=
     first for this feature to work. Example:

     #+BEGIN_SRC clojure
     (cfg/enable-functions-as-defaults config-comp)
     (cfg/define config-comp {:host {:type :string}
                             :port {:type :number}
                             :connstring {:type :string
                                          :default #(str (cfg/get config-comp :host) ":" (cfg/get config-comp :port))}})
     (cfg/populate-from-map config-comp {:host "localhost", :port 8888})
     (cfg/get config-comp :connstring) ;; => "localhost:8888"
     #+END_SRC
   ...
   - =:required= --- if true, the value for this option must be provided,
     ...
     #+BEGIN_SRC clojure
     (cfg/define config-comp {:storage   {:one-of [:file :s3]}
                              :s3-bucket {:required #(= (cfg/get config-comp :storage) :s3)}})
     #+END_SRC
...
(cfg/define config-comp
  {:statsd {:nested {:host {:type :string
     ...
     In the program you can use =cfg/get config-comp= to fetch a concrete value, or a whole
     map at any level:

     #+BEGIN_SRC clojure
     (cfg/get config-comp :statsd :port) ;=> 8125
     (cfg/get config-comp :statsd) ;=> {:host "127.0.0.1", :port 8125}
     #+END_SRC
...
(cfg/populate-from-ssm config-comp "/prod/myapp/")
   #+END_SRC
...
(cfg/populate-from-ssm-continually config-comp "/prod/myapp/" 10)
    #+END_SRC
...
*** Useful functions and macros
    ...
    #+BEGIN_SRC clojure
(cfg/with-options config-comp [username password]
  ;; Binds (cfg/get config-comp :username) to username, and (cfg/get config-comp :password) to password.
  ...)
    #+END_SRC
...
*** Special operations for EDN options
    ...
    #+BEGIN_SRC clojure
    (cfg/define config-comp {:emails {:type :edn
...
(cfg/set-logging-fn config-comp (fn [& args] (log/info (str/join " " args))))
    #+END_SRC
